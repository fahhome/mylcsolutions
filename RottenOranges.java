...
// Alternate Solution queue bfs using level indicator where the queue contains the rotten cells
class RottenOranges {
    ...
    public static boolean isSafe(int i, int j, List<List<Integer>> state) {
        ...
        if ((j - 1 >= 0 && state.get(i).get(j - 1) == 2) || (j + 1 < n && state.get(i).get(j + 1) == 2)
                || (i - 1 >= 0 && state.get(i - 1).get(j) == 2)
    ...
    public static List<List<Integer>> propagateRot(List<List<Integer>> state, int m, int n) {
        ...
        // List<List<Integer>> newState = new LinkedList<>(state); If you pass like this
        // it modifies the state list also
